...
| Success | Confident, concise | Confirm observed state and next action | `Execution completed. Inspect the evidence before drawing a conclusion.` |
| Warning | Cautionary, actionable | Explain risk and safe alternative | `This action may consume your configured budget. Continue?` |
| Blocker/error | Calm, procedural | Reason + recovery + fallback in one block | `Execution cannot start until preflight passes. Review the HOLD and correct the listed requirement.` |
