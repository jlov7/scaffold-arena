...
export async function mockRunPreflight(page: Page): Promise<void> {
  await page.route('**/api/preflight', async (route: Route) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
...
export async function mockRunDiagnostics(page: Page): Promise<void> {
  await page.route('**/api/runs/run_hist_1/diagnostics', async (route: Route) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
...
export async function mockAutopsy(page: Page): Promise<void> {
  await page.route('**/api/autopsy', async (route: Route) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
        failures: [],
        patch: {},
        summary: 'Synthetic test fixture: no concrete failures extracted.',
      }),
    })
  })
}
...
export async function mockDefaultApi(page: Page): Promise<void> {
  ...
  await mockRunPreflight(page)
  await mockRunDiagnostics(page)
  await mockAutopsy(page)
}
