...
describe('theme color contrast', () => {
  ...
  test('light Workbench semantic status tokens use AA-compliant accents', () => {
    const lightBlock = css.match(/:root\[data-theme='light']\s*{([\s\S]*?)}/)?.[1] ?? ''
    expect(lightBlock).toContain('--lab-info: var(--color-accent-info);')
    expect(lightBlock).toContain('--lab-signal: var(--color-accent-winner);')
    expect(lightBlock).toContain('--lab-warning: var(--color-accent-warning);')
    expect(lightBlock).toContain('--lab-danger: var(--color-accent-loser);')
    expect(contrastRatio(light['accent-info'], light['bg-primary'])).toBeGreaterThanOrEqual(4.5)
    expect(contrastRatio(light['accent-warning'], light['bg-primary'])).toBeGreaterThanOrEqual(4.5)
    expect(contrastRatio(light['accent-loser'], light['bg-primary'])).toBeGreaterThanOrEqual(4.5)
  })
})
