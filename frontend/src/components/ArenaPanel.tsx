...
import { formatApiErrorMessage } from '../errors/classify'
...
function ArenaPanelComponent({ panel }: ArenaPanelProps) {
  ...
  const outputText = typeof panel.output === 'string' ? panel.output : ''
  const streamingText = typeof streamedText === 'string' ? streamedText : ''
  const copyableText = outputText || streamingText
  ...
  const displayError = error ? formatApiErrorMessage(error) : null
    ...
    >
      ...
      <div className="mb-3 flex-1 overflow-hidden">
        {status === 'idle' ? (
          ...
          <StreamingText text={streamingText} isStreaming={isRunning} />
        )}
      </div>
