...
import { classifyApiError, formatApiErrorMessage } from './errors/classify'
import {
  getTelemetryConsent,
  readTrackedEvents,
  setTelemetryConsent,
...
import { useAutopsy } from './features/autopsy/useAutopsy'
import { useComparison } from './features/comparison/useComparison'
import { useRunHistory } from './features/history/useRunHistory'
import { useReportExports } from './features/reporting/useReportExports'
import { useOnboardingTelemetry } from './features/onboarding/useOnboardingTelemetry'
import { useRunCompletion } from './features/runs/useRunCompletion'
import type {
  AppMeta,
  RunResults,
} from './types'
...
function getErrorMessage(err: unknown): string {
  return formatApiErrorMessage(getRawErrorMessage(err))
}
...
function hasConfiguredApiToken(tokenValue?: string): boolean {
  const token = tokenValue ?? ''
  return token.trim().length > 0
}

function hasScoredResults(results: RunResults | null): boolean {
  if (!results) return false
  return Object.values(results).some(
    (result) => Boolean(result?.evaluation) && !result.error,
  )
}
...
export default function LegacyApp() {
  ...
  useEffect(() => {
    if (runError) {
      pushToast('error', getErrorMessage(runError))
    }
  }, [runError, pushToast])
  ...
  useEffect(() => {
    const errorMessage = operationError ?? (runError ? getErrorMessage(runError) : null)
  ...
  const {
    ...
    pushToast,
    setOperationError,
    onStart: () => setAutopsyGuideDismissed(true),
  })
  const {
    exportBundle,
    exportJson,
  ...
  const estimatedCostUsd = useMemo(() => {
    ...
    const estimates: Record<string, { input: number; output: number }> = {
      ...
      tool_use_recovery: { input: 7200, output: 2600 },
      privacy_boundary: { input: 6200, output: 2000 },
      strategy_to_system: { input: 9500, output: 3600 },
    }
    ...
    (taskId: string, modelId: string) => {
      ...
      if (runMode === 'model') {
        ...
        void createModelComparison({
          ...
          .then((result) => {
            beginComparisonStream(result.stream_url)
          })
          .catch((err) => {
            resetComparison()
      ...
      void runPreflight({
        ...
        .catch((err) => {
          ...
          if (classifyApiError(raw).kind === 'auth') {
  ...
  }, [closeAutopsy, openHelpCenter, runFromCurrentSelection, setReportOpen])
  ...
  const hasSuccessfulFinalResults = hasScoredResults(finalResults)
  const journey = useJourneyProgress({
    isRunning,
    hasEverRun,
    finalResults: hasSuccessfulFinalResults ? finalResults : null,
  ...
  const hasResults = hasSuccessfulFinalResults || onboardingProgress.reviewCompleted
  const hasComparison =
    comparisonCases.length > 0 || onboardingProgress.comparisonCompleted
  const hasApiToken = hasConfiguredApiToken(apiToken)
  const activeErrorMessage =
    operationError ?? (runError ? getErrorMessage(runError) : null)
  const resultsUiState = useMemo(
    () =>
      describeResultsWorkspaceState({
        isRunning,
        hasResults: hasSuccessfulFinalResults,
    ...
    [activeErrorMessage, connectionState, hasSuccessfulFinalResults, isOnline, isRunning],
  )
  const nextActionKey: NextActionKey = useMemo(() => {
    return resolveNextActionKey({
      ...
      hasResults: hasSuccessfulFinalResults,
      hasComparison,
      hasBlockedRun: Boolean(finalResults && !hasSuccessfulFinalResults),
    })
  }, [finalResults, hasComparison, hasModel, hasRun, hasSuccessfulFinalResults, hasTask])
  ...
  const roleResultsSummary = useMemo<RoleResultsSummary | null>(
    () =>
      buildRoleResultsSummary({
        results: hasSuccessfulFinalResults ? finalResults : null,
        winnerId,
        scaffoldNames,
        userProfile,
      }),
    [finalResults, hasSuccessfulFinalResults, scaffoldNames, userProfile, winnerId],
  )
  ...
  const submitFrictionFeedback = useOnboardingTelemetry({
    helpOpen,
    helpSource,
    blocker: currentPlaybook.blocker,
  ...
  if (metaError) {
    return (
      <main id="main-content" className="min-h-screen bg-bg-primary flex items-center justify-center font-mono">
      ...
      <div className="space-y-6">
        ...
        {activeView === 'results' && (
          <ResultsWorkspace
            ...
            onRunComparison={runComparison}
            onRunAutopsy={runAutopsyForScaffold}
            onExportReport={exportReport}
          ...
          <SettingsWorkspace
            ...
            onClearLlmKey={() => {
              const confirmed = window.confirm(
                'Clear the API key from this session?',
              )
              if (!confirmed) return
              setLlmApiKeyState('')
              setLlmApiKey('')
            ...
            onClearApiToken={() => {
              const confirmed = window.confirm(
                'Clear the local API token from this session?',
              )
              if (!confirmed) return
              setApiTokenState('')
              persistApiToken('')
