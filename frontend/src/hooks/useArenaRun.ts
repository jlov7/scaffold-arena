...
function hasResultError(result: RunResults[string] | undefined): boolean {
    ...
    (eventName: string, data: unknown) => {
      ...
      switch (eventName) {
        ...
        case 'run_complete': {
          ...
          setPanels((prev) =>
            prev.map((p) => {
              const result = e.results?.[p.scaffoldId]
              if (p.status === 'failed' || hasResultError(result)) {
                return {
                  ...p,
                  status: 'failed',
                  error: result?.error ?? p.error,
                }
              }
    ...
    (
      ...
      setPanels((prev) =>
        prev.map((panel) => {
          ...
          const status = hasResultError(result)
            ? 'failed'
            : panel.scaffoldId === hydratedWinnerId
              ? 'winner'
              : ('loser' as const)
          return {
            ...panel,
            ...
            streamedText: typeof result.output === 'string' ? result.output : '',
