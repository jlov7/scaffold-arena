...
describe('useArenaRun', () => {
  ...
  it('hydrates failed result records as failed panels instead of loser panels', () => {
    const { result } = renderHook(() =>
      useArenaRun([{ id: 'bare', name: 'Bare', subtitle: '' }]),
    )

    act(() => {
      result.current.hydrateFromResults(
        { bare: { error: 'Provider key rejected' } } as unknown as RunResults,
        null,
      )
    })

    expect(result.current.panels[0].status).toBe('failed')
    expect(result.current.panels[0].error).toBe('Provider key rejected')
    expect(result.current.panels[0].streamedText).toBe('')
  })
})
