...
type TaskKind = 'extraction' | 'risk' | 'research' | 'tool_use_recovery' | 'privacy_boundary' | 'strategy_to_system' | 'custom'
...
const TASK_STEPS: Record<TaskKind, string[]> = {
  ...
  tool_use_recovery: [
    'Confirm selected tools are available before executing the plan.',
    'Check recovery steps for retry, confirmation, and safe fallback behavior.',
    'Make sure unavailable or forbidden tools are avoided in the execution sequence.',
  ],
  privacy_boundary: [
    'Check that raw identifiers, secrets, and excluded personal data are not repeated.',
    'Verify the output keeps only operationally necessary facts.',
    'Confirm redactions are explicit enough for audit review.',
  ],
  strategy_to_system: [
    'Confirm every required planning section is present and non-empty.',
    'Check that risks, eval plan, backlog, metrics, and open questions are concrete.',
    'Verify strategy concepts are compiled into system decisions rather than restated.',
  ],
  custom: [
...
function normalizeTask(taskId: string): TaskKind {
  if (
    ...
    taskId === 'risk' ||
    taskId === 'research' ||
    taskId === 'tool_use_recovery' ||
    taskId === 'privacy_boundary' ||
    taskId === 'strategy_to_system'
  ) {
    return taskId
  }
  return 'custom'
