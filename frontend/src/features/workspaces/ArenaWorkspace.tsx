...
export function ArenaWorkspace({
  ...
  const hasScoredFinalResults = useMemo(
    () =>
      finalResults
  ...
  return (
    <>
      ...
      <PrimaryCtaRail
        ...
        hasResults={hasScoredFinalResults}
      ...
      {arenaLane === 'live' && (
        <div className="space-y-6" id="arena-grid">
          ...
          {finalResults && !isRunning && hasScoredFinalResults && (
            <StateCallout
              kind="success"
              title="Run complete"
          ...
          {finalResults && !isRunning && !hasScoredFinalResults && (
            <StateCallout
              kind="blocked"
              title="Run ended without usable outputs"
              description="No scaffold produced scored evidence. Verify provider credentials in Settings before treating this run as benchmark evidence."
              actionLabel="Open settings"
              onAction={() => onNavigateToView('settings')}
            />
          )}
        </div>
      )}
