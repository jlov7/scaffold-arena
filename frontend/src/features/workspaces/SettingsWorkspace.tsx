...
import type { RunPreflightResult } from '../../api/client'
    ...
    <section className="max-w-5xl space-y-4 text-sm">
      ...
      <div className="lab-panel border-accent-info/30 p-4">
        ...
        <div className="mt-1 text-sm text-text-muted">
          Enter your own API key to run benchmarks. It stays only in memory for this browser session and is never sent to our server for storage.
        </div>
        ...
        <div className="mt-1.5 text-[10px] text-text-muted">
          {llmApiKey ? 'Key configured; sent per-request for this session.' : 'No key set; server-side keys will be used if available'}
        </div>
      </div>

      <div className="lab-panel border-accent-warning/30 p-4">
        <div className="text-sm font-semibold text-accent-warning">
          Local API Session Token
        </div>
        <div className="mt-1 text-sm text-text-muted">
          Required only when the local backend has `API_SECRET_KEY` enabled. This
          is separate from provider keys and is held only for this browser session.
        </div>
        <div className="mt-2 flex flex-col gap-2 sm:flex-row sm:items-center">
          <input
            ...
            placeholder="Bearer token for this local backend"
            autoComplete="off"
            spellCheck={false}
            className="ui-control min-w-0 flex-1 rounded-md border border-border bg-bg-secondary px-3 py-2 text-sm text-text-primary placeholder:text-text-muted focus:border-accent-info focus:outline-none"
          />
          {apiToken && (
            <button
              type="button"
        ...
        <div className="mt-1.5 text-[10px] text-text-muted">
          {hasApiToken
            ? 'Session token configured via environment or this browser.'
            : 'No session token configured; protected endpoints will block runs.'}
        </div>
      </div>
      ...
      <div className="mt-3 grid gap-3 sm:grid-cols-2">
        ...
        <div className="rounded border border-border/60 bg-bg-primary p-3">
          ...
          <div className="mt-1 text-[11px] text-text-muted">
            Enter a local API token for this session before running protected actions.
          </div>
        </div>
        <div className="rounded border border-border/60 bg-bg-primary p-3">
