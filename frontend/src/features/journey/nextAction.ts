...
export function resolveNextActionKey(params: {
  ...
  const { hasTask, hasModel, hasRun, hasResults, hasComparison, hasBlockedRun = false } = params
  if (!hasTask) return 'pick_task'
  if (!hasModel) return 'pick_model'
  if (hasBlockedRun) return 'resolve_blocker'
...
export function buildNextActionCopy(
  ...
  const base: Record<NextActionKey, NextActionCopy> = {
    ...
    resolve_blocker: {
      title: 'Resolve run blocker',
      body: 'Provider credentials blocked usable evidence. Open Settings, update keys, then rerun.',
      cta: 'Open settings',
    },
    run: {
      title: 'Run the arena',
