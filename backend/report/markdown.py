...
def generate_markdown(
    ...
    for scaffold_name, scaffold_data in results.items():
        ...
        cost_usd = _reported_cost(metrics.get("cost_usd"))
        ...
        rows.append(
            ...
            f" {'UNKNOWN' if cost_usd is None else f'${cost_usd:.4f}'} | {wall_time_ms:,}ms | {num_api_calls} |"
        )
    ...
    if scores:
        ...
        cost_range = f"${min(costs):.4f}–${max(costs):.4f}" if costs else "UNKNOWN"
        auto_summary = (
            f"Winner: **{winner}** with score {winner_score:.2f}. "
            f"Cost range: {cost_range}."
        )
    else:
...
def _build_recommendations(results: dict, scores: dict) -> list[str]:
    ...
    cost_map = {
        name: _reported_cost(data.get("metrics", {}).get("cost_usd"))
        for name, data in results.items()
    }
    reported_costs = {name: cost for name, cost in cost_map.items() if cost is not None}
    if winner in reported_costs:
        cheapest = min(reported_costs, key=lambda name: reported_costs[name])
        ...
        if cheapest != winner and cheapest_score >= winner_score * 0.9:
            recs.append(
                ...
                f"(${reported_costs[cheapest]:.4f} vs ${reported_costs[winner]:.4f}, "
                f"score within 10% of winner)."
            )
        elif cheapest != winner:
            recs.append(
                f"**{cheapest}** has the lowest cost (${reported_costs[cheapest]:.4f}) "
