...
async def test_start_arena_run_handles_provider_init_failure(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert run.results["bare"]["error"] == (
        "Provider key missing. Set a valid provider key in Settings or "
        "backend environment, then retry."
    )
    assert run.results["plan_execute_verify"]["error"] == run.results["bare"]["error"]
...
@pytest.mark.asyncio
async def test_start_arena_run_sanitizes_runtime_provider_auth_failure(
    ...
    class FailingProvider:
        async def stream_text(self, *args: object, **kwargs: object):
            del args, kwargs
            raise RuntimeError(
        ...
        async def complete(self, *args: object, **kwargs: object) -> object:
            del args, kwargs
            raise RuntimeError("unexpected complete call")

        def get_last_usage(self) -> object:
            return object()

    monkeypatch.setattr(
        "core.run_engine.get_provider",
        lambda model_id, api_key_override=None: FailingProvider(),
    )

    caplog.set_level("ERROR")
    await start_arena_run(run)

    error = run.results["bare"]["error"]
    assert error == (
        "Provider key rejected (401). Update the model provider key in Settings "
        "or backend environment, then retry."
    )
    assert "invalid x-api-key" not in error
    assert "req_sensitive" not in error
    assert "invalid x-api-key" not in caplog.text
    assert "req_sensitive" not in caplog.text

    failed_events: list[dict] = []
    while not run.queue.empty():
        event_type, event_data = run.queue.get_nowait()
        if event_type == "scaffold_failed":
            failed_events.append(event_data)

    assert failed_events[0]["error"] == error
