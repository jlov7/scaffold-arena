...
def test_all_models_meta_contains_price_keys() -> None:
    ...
    for entry in models:
        ...
        assert entry["price_catalog_revision"] == PRICE_CATALOG_REVISION
        assert entry["price_source_url"].startswith("https://")


def test_price_catalogue_is_versioned_and_uses_current_openrouter_and_gemini_tiers() -> None:
    assert cost_usd(1_000_000, 1_000_000, "openrouter/openai/gpt-4.1") == pytest.approx(10.0)
    assert cost_usd(1_000_000, 1_000_000, "openrouter/openai/gpt-4.1-mini") == pytest.approx(2.0)
    standard = get_price_catalog_entry(provider="gemini", model_id="gemini-2.5-pro")
    long_context = get_price_catalog_entry(provider="gemini", model_id="gemini-2.5-pro", billing_profile="long_context")
    assert standard.billing_profile.max_context_tokens == 200_000
    assert long_context.billing_profile.min_context_tokens == 200_001
