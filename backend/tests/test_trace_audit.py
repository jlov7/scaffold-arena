...
def test_sample_fixture_exercises_public_release_taxonomy() -> None:
    fixture_path = Path(__file__).parent / "fixtures/trace_audit/sample_runs.json"
    records = load_trace_records(fixture_path)
    report = audit_records(records)
    finding_types = {finding.type for finding in report.findings}

    assert "unsupported_claims" in finding_types
    assert "privacy_boundary_violation" in finding_types
    assert "scaffold_overreach" in finding_types
    assert "regression_from_previous_best" in finding_types
    assert "weak_evidence_trail" in finding_types
    assert all(finding.suggested_fixture for finding in report.findings)
